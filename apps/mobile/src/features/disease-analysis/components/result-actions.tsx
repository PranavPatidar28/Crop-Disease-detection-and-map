...
export function ResultActions({ report, onUploadAnother, onViewOnMap }: ResultActionsProps) {
  ...
  const handleShare = async () => {
    ...
    try {
      await Share.share({
        message: `Crop diagnosis from Crop Disease Mapping\n\n${report.cropType} · ${
  ...
  return (
    <View className="flex-row items-stretch gap-2">
      <ActionButton
        icon={<Map size={18} color={theme.text} strokeWidth={2.2} />}
        label="View on map"
        onPress={() => {
      ...
      />
      <ActionButton
        icon={<Share2 size={18} color={theme.text} strokeWidth={2.2} />}
        label="Share"
        onPress={handleShare}
      />
      <ActionButton
        icon={<Plus size={18} color={theme.text} strokeWidth={2.2} />}
        label="New report"
        onPress={() => {
          Haptics.selectionAsync().catch(() => undefined);
          onUploadAnother();
  ...
  return (
    ...
    >
      <View className="items-center gap-1.5 rounded-2xl border border-border bg-surface px-3 py-3">
        {icon}
        <Text className="text-[11px] font-semibold text-text" numberOfLines={1}>
          {label}
        </Text>
      </View>
    </Pressable>
